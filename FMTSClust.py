...
def getFAWNdata(path):
    ...
    # Drop Feb-29 of leap years: 2000, 2004, 2008, 2012, 2016, 2020
...
def transform_dfTS_to_3Darray_DOYsorted(df, columns, doy_lim1, doy_lim2):
    ...
    for station_id in unique_station_ids:
        ...
        firstMonthsYear = df_temp[df_temp["doy"] <= doy_lim1] # until may
        lastMonthsYear = df_temp[df_temp["doy"] >= doy_lim2] # from november
...
def plot_map_clustering_names(stations, cluster_labels, title):
    ...
    for index, row in df_plot.iterrows():
        st_name = row['StationName'].rsplit(' (')[0]
        plt.text(row['longitude']-0.1,row['latitude']+0.1, st_name, horizontalalignment='center', size='medium', color='black')
